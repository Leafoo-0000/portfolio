...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    ...
    document.querySelector(".term-bucket")
  ) {
...
function renderHomepageShowcase() {
  ...
  const featuredAchievements = PORTFOLIO_DATA.achievements.filter(
    (a) => a.featured,
  );
...
function createCardHTML(item) {
  const imageMarkup = item.image
    ? `<img src="${item.image}" alt="${item.title}" style="width:100%; border-radius:6px; margin-bottom:10px; object-fit:cover;" />`
    : "";
  const titleMarkup = item.link
    ? `<a href="${item.link}" target="_blank" class="tile-title-link">${item.title}</a>`
    : item.title;
  ...
  const tagBadges = item.tags
    .map((t) => `<span class="tile-tag-badge">${t}</span>`)
    .join("");
  ...
  function executeFilterCycle() {
    ...
    cards.forEach((card) => {
      ...
      const matchesCategory =
        activeCategory === "all" || typeLabel === activeCategory;
...
function initGlobalTheme() {
  ...
  try {
    ...
    console.warn(
      "Storage engine restricted locally. Running application in fallback runtime mode.",
    );
  }
